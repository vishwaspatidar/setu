// Backend/src/routes/user.routes.js
import { Router } from "express";
import { UserDetails } from "../controllers/user.controllers.js";
import { verifyJWT_email, verifyJWT_username } from "../middlewares/verifyJWT.middleware.js";
import verifyOptionalJWT from "../middlewares/verifyOptionalJWT.middleware.js";
import {
  UnRegisteredUserDetails,
  saveRegUnRegisteredUser,
  saveEduUnRegisteredUser,
  saveAddUnRegisteredUser,
  registerUser,
  userDetailsWithoutID,
  saveRegRegisteredUser,
  saveEduRegisteredUser,
  saveAddRegisteredUser,
  uploadPic,
  discoverUsers,
  sendScheduleMeet,
} from "../controllers/user.controllers.js";
import { upload } from "../middlewares/multer.middleware.js";

const router = Router();

// NOTE:
// For the unregistered / registration flow we use verifyOptionalJWT so:
// - requests with a valid token are verified and req.user is populated
// - requests without a token are allowed to continue for manual registration
router.route("/unregistered/getDetails").get(verifyOptionalJWT, UnRegisteredUserDetails);
router.route("/unregistered/saveRegDetails").post(verifyOptionalJWT, saveRegUnRegisteredUser);
router.route("/unregistered/saveEduDetail").post(verifyOptionalJWT, saveEduUnRegisteredUser);
router.route("/unregistered/saveAddDetail").post(verifyOptionalJWT, saveAddUnRegisteredUser);
router.route("/registerUser").post(verifyOptionalJWT, registerUser);

// update user details (these remain protected - require authenticated username)
router.route("/registered/saveRegDetails").post(verifyJWT_username, saveRegRegisteredUser);
router.route("/registered/saveEduDetail").post(verifyJWT_username, saveEduRegisteredUser);
router.route("/registered/saveAddDetail").post(verifyJWT_username, saveAddRegisteredUser);
// router.route("/registered/updateDetails").post(verifyJWT_username, updateRegisteredUser);

// Upload Picture (protected)
router
  .route("/uploadPicture")
  .post(verifyJWT_username, upload.fields([{ name: "picture", maxCount: 1 }]), uploadPic);

// get user details (protected)
router.route("/registered/getDetails/:username").get(verifyJWT_username, UserDetails);
router.route("/registered/getDetails").get(verifyJWT_username, userDetailsWithoutID);

// get profiles for discover page (protected)
router.route("/discover").get(verifyJWT_username, discoverUsers);

// send schedule meet email (protected)
router.route("/sendScheduleMeet").post(verifyJWT_username, sendScheduleMeet);

export default router;
